...
unsigned int Rcon[4][10] = { {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36},
                       {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
                       {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
                       {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00} };
/* defines */
/* hardware */
#define to_hw_port (volatile uint32_t*) 0x40000000 // actual address here
#define to_hw_sig  (volatile uint8_t*)  0x40000010 // actual address here
#define to_sw_port (volatile uint32_t*) 0x40000020 // actual address here
#define to_sw_sig  (volatile uint8_t*)  0x40000030 // actual address here

/* aes */
#define _NB  4  // block size
#define _NK  4  // key length
#define _NR 10  // number of rounds

// Decrypted message
uint8_t decryptedMessage[33];

uint8_t MIX_COLUMNS_MATRIX[_NB][_NB] = {
    {2, 3, 1, 1},
    {1, 2, 3, 1},
    {1, 1, 2, 3},
    {3, 1, 1, 2}
...
uint8_t TEST_KEY_EXPANSION[_NB][_NB] = {
    {0x2B, 0x28, 0xAB, 0x09},
    {0x7E, 0xAE, 0xF7, 0xCF},
    {0x15, 0xD2, 0x15, 0x4F},
    {0x16, 0xA6, 0x88, 0x3C}
...
uint8_t TEST_MIX_COLUMNS[_NB][_NB] = {
    {0xD4, 0xE0, 0xB8, 0x1E},
    {0xBF, 0xB4, 0x41, 0x27},
    {0x5D, 0x52, 0x11, 0x98},
    {0x30, 0xAE, 0xF1, 0xE5}
...
uint8_t TEST_SUB_BYTES[_NB][_NB] = {
    {0x19, 0xA0, 0x9A, 0xE9},
    {0x3D, 0xF4, 0xC6, 0xF8},
    {0xE3, 0xE2, 0x8D, 0x48},
    {0xBE, 0x2B, 0x2A, 0x08}
...
uint8_t TEST_SHIFT_ROWS[_NB][_NB] = {
    {0xD4, 0xE0, 0xB8, 0x1E},
    {0x27, 0xBF, 0xB4, 0x41},
    {0x11, 0x98, 0x5D, 0x52},
    {0xAE, 0xF1, 0xE5, 0x30}
...
uint8_t TEST_ADD_ROUND_KEY[_NB][_NB] = {
    {0x04, 0xE0, 0x48, 0x28},
    {0x66, 0xCB, 0xF8, 0x06},
    {0x81, 0x19, 0xD3, 0x26},
    {0xE5, 0x9A, 0x7A, 0x4C}
...
//    scanf("%2000s[^\n]", keyText);
	...
	*to_hw_sig = 1;
    ...
    // TEST CASES ---------------------------------------------------------------
    // // test sub bytes
    // subBytes(TEST_SUB_BYTES);
    // for(int row = 0; row < _NB; row++) {
    //     for(int col = 0; col < _NB; col++) {
    //         printf("%02x ", TEST_SUB_BYTES[row][col]);
    //     }
    //     printf("\n");
    // }

    // // test shift rows
    // shiftRows(TEST_SHIFT_ROWS);
    // for(int row = 0; row < _NB; row++) {
    //     for(int col = 0; col < _NB; col++) {
    //         printf("%02x ", TEST_SHIFT_ROWS[row][col]);
    //     }
    //     printf("\n");
    // }

    // // test mix columns
    // mixColumns(TEST_MIX_COLUMNS);
    // for(int row = 0; row < _NB; row++) {
    //     for(int col = 0; col < _NB; col++) {
    //         printf("%02x ", TEST_MIX_COLUMNS[row][col]);
    //     }
    //     printf("\n");
    // }
    ...
    // // test key expansion
    // uint8_t roundKeys[_NB][_NB * (_NR + 1)];
    // keyExpansion(TEST_KEY_EXPANSION, roundKeys);
    // for(int row = 0; row < _NB; row++) {
    //     for(int col = 0; col < (_NB * (_NR + 1)); col++) {
    //         printf("%02x ", roundKeys[row][col]);
    //     }
    //     printf("\n");
    // }
    // addRoundKey(TEST_ADD_ROUND_KEY, roundKeys, 4);
    // for(int row = 0; row < _NB; row++) {
    //     for(int col = 0; col < _NB; col++) {
    //         printf("%02x ", TEST_ADD_ROUND_KEY[row][col]);
    //     }
    //     printf("\n");
    // }

    // GIVEN CODE ---------------------------------------------------------------
    // uint8_t plaintext[33]; //should be 1 more character to account for string terminator
    // uint8_t key[33];
